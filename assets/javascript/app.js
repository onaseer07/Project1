...
$(document).ready(function () {
    ...
    $("#submit").on("click", function (event) {
        ...
        $.ajax(settings).done(function (response) {
            ...
            var BGScreen2 = $(`<div style="background-image:url(https://images.igdb.com/igdb/image/upload/t_screenshot_big/${response[0].screenshots[0].cloudinary_id}.jpg);
            height:290px;width:100%;position:absolute;overflow:hidden;background-size:cover;right:0;filter:blur(5px);
            background-position-y:-560px;top: 865px;"></div>`).addClass('bg-image')
            $('.ScreenContainer').append(BGScreen2)
            ...
            $('#SimilarGames').prepend(`<div id="SimGamesTitle">Similar Games</div>`)
            var TwitchSettings = {
                "async": true,
                "crossDomain": true,
            ...
            $.ajax(TwitchSettings).done(function (response) {
                ...
                if (response.streams.length !== 0) {
                    $('#Twitch').prepend(`<div id="LSTitle">Live Stream</div>`)
                    $('#Twitch').append(`<iframe id="LS" height=300px width=100% src="http://player.twitch.tv/?channel=${response.streams[0].channel.name}&autoplay=false" allowfullscreen frameborder=0><iframe>`)
                    $('#SimilarGames').css('left', '215px');
                } else {
                    $('#SimilarGames').css('left', '215px');
                }
            ...
            var NewSimilar = SimilarGamesIdList.toString();

            var settings2 = {
                "async": true,
                "crossDomain": true,
                "url": "https://cors-anywhere.herokuapp.com/https://api-endpoint.igdb.com/games/" + NewSimilar + "?fields=*",
                "method": "GET",
                "headers": {
                    "user-key": "186a3663d438fb1004a180d435b09d81",
                    "accept": "application/json"
            ...
            <img class="SimGameThumb d-block w-100" height="300" width="240" data-id="${response[0].id}" src="https://images.igdb.com/igdb/image/upload/t_cover_med/${response[0].cover.cloudinary_id}.jpg">
            ...
            <p id="CText">${response[0].name}</p>
                ...
                for (var s = 1; s < response.length; s++) { //loop to show each cover thumbnail of similar games, might change to title and cover
                    ...
                    <img class="SimGameThumb d-block w-100" height="300" width="240" data-id="${response[s].id}" src="https://images.igdb.com/igdb/image/upload/t_cover_big/${response[s].cover.cloudinary_id}.jpg">
                    <div class="carousel-caption d-none d-sm-block">
